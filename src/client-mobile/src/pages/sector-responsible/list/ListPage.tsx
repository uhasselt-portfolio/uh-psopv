...
class ListView extends Component<any> {
                ...
                <IonRow>
                    <IonCol>
                        <IonButton className="floatLeft">
                            <IonSelect
                                interface="popover"
                                value={this.state.selected_sort} placeholder={this.state.selected_sort}
                                onIonChange={e => this.handleSortChange(e.detail.value)}>
                                <IonSelectOption
                                    value={sort_types.alfabetisch}> {sort_types.alfabetisch}</IonSelectOption>
                                <IonSelectOption value={sort_types.afstand}> {sort_types.afstand}</IonSelectOption>
                                <IonSelectOption
                                    value={sort_types.best_route}> {sort_types.best_route}</IonSelectOption>
                            </IonSelect>
                        </IonButton>
                    </IonCol>
                    <IonCol>
                                ...
                                {this.props.localStorage.posts_sectors.map((sector: any) => {
                                    if (this.state.default_sector === sector) {
                                        return <IonSelectOption
                                            value={sector.sector_id}>Sector {sector.sector_id}(D) </IonSelectOption>
                                    } else {
                                        return <IonSelectOption
                                            value={sector.sector_id}>Sector {sector.sector_id}</IonSelectOption>
                                    }
                                })}
    ...
    getRouteFromEdges(best_route: any) {
        ...
        for (let i = 0; i < route_length; i++) {
            edge = best_route.find((edge: any) => {
                return ((edge.start_vertex.post_id === post.post_id && edge.end_vertex.post_id !== prev_post.post_id) ||
