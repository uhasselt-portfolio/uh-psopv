...
export const fetchPlanning = () => async (dispatch : Redux.Dispatch) => {
    ...
    try {
        ...
        const responseShifts = await axios.get('http://localhost/api/shift/fetch/all'); //TODO not authorized
        const responseItems = await axios.get('http://localhost/api/item/fetch/all');

        console.log(responseShifts);
        console.log(responseItems);

        dispatch({
            type: PlanningActions.PLANNING_FETCH_SUCCES,
            payload: {
                shifts: responseShifts.data.data.shifts,
