...
export default function (state : State = initialState, action : AnyAction) : State {
    switch(action.type) {
        case DetailActions.DETAIL_PLANNING_FETCH_START:
            return {...state, loading: true, isPlanningFetched: false, isMapFetched: false, errorMessage: ""}
        ...
        case DetailActions.DETAIL_PLANNING_FETCH_FAIL:
            return {...state, loading: false, isPlanningFetched: false, isMapFetched: false, errorMessage: action.payload}
        default:
            return state
    }
}